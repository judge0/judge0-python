from base64 import b64decode
from datetime import timezone
from typing import cast
from unittest.mock import Mock
from uuid import UUID

import pytest

from judge0 import Status, Submission, run, wait
from judge0.base_types import LanguageAlias
from judge0.clients import Client


def test_from_json() -> None:
    submission_dict = {
        "source_code": "cHJpbnQoJ0hlbGxvLCBXb3JsZCEnKQ==",
        "language_id": 100,
        "stdin": None,
        "expected_output": None,
        "stdout": "SGVsbG8sIFdvcmxkIQo=",
        "status_id": 3,
        "created_at": "2024-12-09T17:22:55.662Z",
        "finished_at": "2024-12-09T17:22:56.045Z",
        "time": "0.152",
        "memory": 13740,
        "stderr": None,
        "token": "5513d8ca-975b-4499-b54b-342f1952d00e",
        "number_of_runs": 1,
        "cpu_time_limit": "5.0",
        "cpu_extra_time": "1.0",
        "wall_time_limit": "10.0",
        "memory_limit": 128000,
        "stack_limit": 64000,
        "max_processes_and_or_threads": 60,
        "enable_per_process_and_thread_time_limit": False,
        "enable_per_process_and_thread_memory_limit": False,
        "max_file_size": 1024,
        "compile_output": None,
        "exit_code": 0,
        "exit_signal": None,
        "message": None,
        "wall_time": "0.17",
        "compiler_options": None,
        "command_line_arguments": None,
        "redirect_stderr_to_stdout": False,
        "callback_url": None,
        "additional_files": None,
        "enable_network": False,
        "post_execution_filesystem": "UEsDBBQACAAIANyKiVkAAAAAAAAAABYAAAAJABwAc"
        "2NyaXB0LnB5VVQJAANvJ1dncCdXZ3V4CwABBOgDAAAE6AMAACsoyswr0VD3SM3JyddRCM8v"
        "yklRVNcEAFBLBwgynNLKGAAAABYAAABQSwECHgMUAAgACADciolZMpzSyhgAAAAWAAAACQA"
        "YAAAAAAABAAAApIEAAAAAc2NyaXB0LnB5VVQFAANvJ1dndXgLAAEE6AMAAAToAwAAUEsFBg"
        "AAAAABAAEATwAAAGsAAAAAAA==",
        "status": {"id": 3, "description": "Accepted"},
        "language": {"id": 100, "name": "Python (3.12.5)"},
    }

    _ = Submission(**submission_dict)


def test_z_timestamps_are_parsed_as_utc() -> None:
    submission = Submission()

    submission.set_attributes({"created_at": "2024-12-09T17:22:55.662Z"})

    assert submission.created_at is not None
    assert submission.created_at.tzinfo is timezone.utc


def test_invalid_encoded_value_is_preserved() -> None:
    submission = Submission(stdout="not base64")

    assert submission.stdout == "not base64"


def test_as_body_rejects_missing_source_code() -> None:
    client = Mock(spec=Client)
    client.get_language_id.return_value = 100

    with pytest.raises(ValueError, match="source_code must be provided"):
        Submission().as_body(cast(Client, client))


def test_batch_tokens_are_stringified() -> None:
    submission = Submission(token=UUID("5513d8ca-975b-4499-b54b-342f1952d00e"))
    transport = Mock()
    response = Mock()
    response.json.return_value = {"submissions": [{}]}
    transport.get.return_value = response
    client = Client.__new__(Client)
    client.client = transport
    client.headers = {}

    client.get_submissions([submission])

    assert transport.get.call_args.kwargs["params"]["tokens"] == str(submission.token)


def test_batch_tokens_reject_missing_values() -> None:
    client = Client.__new__(Client)
    client.client = Mock()
    client.headers = {}

    with pytest.raises(ValueError, match="token"):
        client.get_submissions([Submission()])


def test_status_before_and_after_submission(ce_client: Client) -> None:
    submission = Submission(
        source_code='print("Hello World!")', language=LanguageAlias.PYTHON
    )

    assert submission.status is None

    ce_client.create_submission(submission)
    ce_client.get_submission(submission)

    assert isinstance(submission.status, Status)
    assert submission.status >= Status.IN_QUEUE


def test_is_done(ce_client: Client) -> None:
    submission = Submission(
        source_code='print("Hello World!")', language=LanguageAlias.PYTHON
    )

    assert submission.status is None

    ce_client.create_submission(submission)
    wait(client=ce_client, submissions=submission)

    assert submission.is_done()


def test_language_before_and_after_execution(ce_client: Client) -> None:
    code = """\
    public class Main {
        public static void main(String[] args) {
            System.out.println("Hello World");
        }
    }
    """

    submission = Submission(
        source_code=code,
        language=LanguageAlias.JAVA,
    )

    assert submission.language == LanguageAlias.JAVA
    submission = run(client=ce_client, submissions=submission)
    assert submission.language == LanguageAlias.JAVA


def test_language_executable(ce_client: Client) -> None:
    code = b64decode(
        ""
    )
    submission = Submission(
        source_code=code,
        language=LanguageAlias.EXECUTABLE,
    )

    assert submission.language == LanguageAlias.EXECUTABLE
    submission = run(client=ce_client, submissions=submission)
    assert submission.language == LanguageAlias.EXECUTABLE
    assert submission.stdout == "hello, world\n"
